...
// demonstrating system out of bounds
fn demo_index_out_of_bound() {
    // when you request input at index 2 when transaction only has index 0
    let ret = ckb_std::high_level::load_cell_lock_hash(2, Source::Input);
    match ret {
...
fn demo_item_missing() {
    ...
    // cell at groupInput[0] has no type script - Type hash missing
    let ret = ckb_std::syscalls::load_cell_by_field(
        &mut buffer, 0, 0,
        Source::GroupInput, CellField::TypeHash,
    );
    match ret {
...
fn demo_length_not_enough() {
    ...
    match ret {
        Err(SysError::LengthNotEnough(_s)) => {
            // _s == 32 the actual size is returned inside the variant
        }
        _ => panic!("unknown"),
    }
}
...
fn ckb_main() -> Result<(), SysError> {
    // the ? propagates system error Up if load_script fails
    let args = ckb_std::high_level::load_script()?.args();

    if args.is_empty() {
        debug!("args len ({}) error", args.len());
        // custom sentinel stored in Unknown(u64)
